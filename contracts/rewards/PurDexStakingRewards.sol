...
contract PurDexStakingRewards is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            // ⚡ Bolt: Inlined earned() calculation using cached rewardPerTokenStored
            // to avoid redundant state reads of totalSupply and rewardRate.
    ...
    function exit() external nonReentrant updateReward(msg.sender) {
        // ⚡ Bolt: Use internal _withdraw and _getReward helpers to prevent
        // triggering nonReentrant and updateReward modifiers multiple times unnecessarily.
