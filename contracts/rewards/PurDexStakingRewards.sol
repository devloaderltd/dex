...
contract PurDexStakingRewards is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    /// @dev Consolidated exit function to optimize gas by applying modifiers (nonReentrant, updateReward)
    /// and the KYC verification check only once instead of redundantly in withdraw() and getReward().
