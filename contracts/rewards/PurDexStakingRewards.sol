...
contract PurDexStakingRewards is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            // Inline earned() calculation using the newly updated rewardPerTokenStored
            // to avoid redundantly calling rewardPerToken() again
            rewards[account] =
                ((_balances[account] * (rewardPerTokenStored - userRewardPerTokenPaid[account])) / 1e18) +
                rewards[account];
