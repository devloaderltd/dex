...
contract PurDexStakingRewards is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            // Bolt optimization: Use cached rewardPerTokenStored to save gas
            // Original code: rewards[account] = earned(account);
            // The original earned() function calculates rewardPerToken() which was just calculated and stored above.
