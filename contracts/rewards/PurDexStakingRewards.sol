...
contract PurDexStakingRewards is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            // Bolt optimization: Inlined the earned() calculation to reuse the freshly updated
            // rewardPerTokenStored, avoiding an expensive, redundant call to rewardPerToken().
    ...
    function exit() external nonReentrant updateReward(msg.sender) {
        // Bolt optimization: Calling internal _withdraw and _getReward helpers
        // directly prevents the nonReentrant and updateReward modifiers from
        // executing redundantly (twice) in a single transaction.
