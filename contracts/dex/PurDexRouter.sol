...
contract PurDexRouter is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    // --- swaps ---
    function _swap(uint256[] memory amounts, address[] memory path, address _to, address _pair) internal {
        for (uint256 i = 0; i < path.length - 1; i++) {
            ...
            // Cache the pair address through the multi-hop loop to avoid redundant external calls to IPurDexFactory.getPair()
            IPurDexPair(_pair).swap(amount0Out, amount1Out, to, new bytes(0));
            _pair = to;
        }
    }
